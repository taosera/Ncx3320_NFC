    ...
    )
{
    ...
    else
    {
        ...
        /* MMa 2016-10-27: to fix ISO-DEP T4T "0x60 issue" at Android HCE */
        // /* Copy SAK */					/*old*/
        // pUidOut[0] = pRcvBuffer[0]; 		/*old*/
        pUidOut[0] = pRcvBuffer[0] & 0xBF;	/*new*/
    }
...
phStatus_t phpalI14443p3a_T1T_Sw_Exchange(
    ...
    )
{
    ...
    phStatus_t   PH_MEMLOC_REM statusTmp;
    uint8_t      PH_MEMLOC_REM n;
    uint8_t      PH_MEMLOC_REM aCrc[2];
    uint8_t      PH_MEMLOC_REM bTemp;


    /* Set  maximum timeout as write operations taking more time. */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(
        pDataParams->pHalDataParams,
        PHHAL_HW_CONFIG_TIMEOUT_VALUE_US,
        5236));
    ...
    bTemp = pTxBuffer[0] ;
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Transmit( pDataParams->pHalDataParams, wOption, &bTemp, 1));
    ...
    for( n = 1; n < wTxLength ; n++ )
    {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Transmit( pDataParams->pHalDataParams, wOption, &pTxBuffer[n], 1));
    }

    /*transmit first byte of the Crc_B*/
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Transmit( pDataParams->pHalDataParams, wOption, &aCrc[0], 1));

    /*Hw_exchange() of last byte of Crc_B*/
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Exchange(pDataParams->pHalDataParams, wOption, &aCrc[1], 1, ppRxBuffer, pRxLength));
