...
/** \file
* Internal functions for Pn512 specific HAL-Component of Reader Library Framework.
* $Author: nxp40786 $
* $Revision: 477 $
                                   ...
                                   )
{
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_ReadRegister(pDataParams, PHHAL_HW_PN512_REG_FIFOLEVEL, &bValue));
    bValue &= (uint8_t)~(uint8_t)PHHAL_HW_PN512_BIT_FLUSHBUFFER;

    /* Check if FiFO has enough space */
    if (wLength > (PHHAL_HW_PN512_FIFOSIZE - bValue))
    {
        /* Truncate byte count */
        wLength = PHHAL_HW_PN512_FIFOSIZE - bValue;
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_WriteData(pDataParams, PHHAL_HW_PN512_REG_FIFODATA, pData, wLength));
                                  ...
                                  )
{
    ...
    /* read out the FiFo Level register */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_ReadRegister(pDataParams, PHHAL_HW_PN512_REG_FIFOLEVEL, &bValue));
    *pLength = (uint16_t)(bValue & (uint8_t)~(uint8_t)PHHAL_HW_PN512_BIT_FLUSHBUFFER);
    ...
    if (*pLength > 0)
    {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_ReadData(pDataParams, PHHAL_HW_PN512_REG_FIFODATA, *pLength, pData));
    }
...
{
    return phhalHw_Pn512_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_FIFOLEVEL, PHHAL_HW_PN512_BIT_FLUSHBUFFER);
}
    ...
    )
{
    ...
    {
        /* Perform actual write to FIFO */
        status = phhalHw_Pn512_WriteFifo(pDataParams, pTmpBuffer, wNumPrecachedBytes, &wFifoBytes);
    ...
    if ((status & PH_ERR_MASK) != PH_ERR_BUFFER_OVERFLOW)
    {
        ...
        {
            /* Perform actual write to FIFO */
            status = phhalHw_Pn512_WriteFifo(pDataParams, pTxBuffer, wNumPrecachedBytes, &wFifoBytes);
    ...
    {
        /* Write the command */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_COMMAND, bCmdCode));
    }

    /* start sending */
    if ((bCmdCode == PHHAL_HW_PN512_CMD_TRANSCEIVE) || (pDataParams->bCardType == PHHAL_HW_CARDTYPE_I18092MPT))
    {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_PN512_REG_BITFRAMING, &bRegister));
        bRegister |= PHHAL_HW_PN512_BIT_STARTSEND;
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_BITFRAMING, bRegister));
    }
    ...
    while ((wTmpBufferLen != 0) || (wTxLength != 0))
    {
        ...
        /* */
#ifdef PHHAL_HW_PN512_FEATURE_FIFO_UNDERFLOW_CHECK
        ...
        if ((bIrq0Rq & PHHAL_HW_PN512_BIT_IDLEI) || (bIrq0Rq & PHHAL_HW_PN512_BIT_TXI))
        {
            /* stop the command */
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_WriteRegister(
                pDataParams,
                PHHAL_HW_PN512_REG_COMMAND,
                PHHAL_HW_PN512_CMD_IDLE));
        ...
        status = phhalHw_Pn512_WriteFifo(pDataParams, pTmpBuffer, wTmpBufferLen, &wFifoBytes);
    ...
    /* Set wait IRQs */
    bIrq0WaitFor = PHHAL_HW_PN512_BIT_TXI | PHHAL_HW_PN512_BIT_IDLEI | PHHAL_HW_PN512_BIT_ERRI;
    bIrq1WaitFor = 0x00;
    ...
    if (bIrq0Rq & PHHAL_HW_PN512_BIT_ERRI)
    {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_PN512_REG_ERROR, &bRegister));

        /* buffer overflow */
        if (bRegister & PHHAL_HW_PN512_BIT_BUFFEROVFL)
        {
            status = PH_ERR_BUFFER_OVERFLOW;
        }
        /* Fifo write error */
        else if (bRegister & PHHAL_HW_PN512_BIT_WRERR)
        {
    ...
    {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_SetConfig(pDataParams, PHHAL_HW_CONFIG_TXLASTBITS, 0x00));
    }
    ...
    )
{
    ...
    /* Set wait IRQs */
    bIrq0WaitFor = PHHAL_HW_PN512_BIT_RXI | PHHAL_HW_PN512_BIT_ERRI | PHHAL_HW_PN512_BIT_HIALERTI | PHHAL_HW_PN512_BIT_TIMERI | PHHAL_HW_PN512_BIT_IDLEI;
    bIrq1WaitFor = 0x00;

    if (wOption != PHHAL_HW_OPTION_FIFO_READ)
    {
        /* wait until the command is finished */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_WaitIrq(
            pDataParams,
        ...
        if (bIrq0Rq & PHHAL_HW_PN512_BIT_TIMERI)
        {
            ...
            {
                PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_FieldReset(pDataParams));
            }
    ...
    /* Incase of Rxmultiple ON, the timer never stops after 
       receiving the first frame. There is always a chance of
       TIMEOUT */
    ...
    if (status == PH_ERR_SUCCESS)
    {
        do
        {
            ...
            while ((!(bIrq0Rq & bIrq0WaitFor)) && (bStatus2 & 0x07));
        ...
        while (((!(bIrq0Rq & (bIrq0WaitFor & (~PHHAL_HW_PN512_BIT_HIALERTI)))) && (bStatus2 & 0x07)) || (wFifoBytes != 0));
        ...
        if (bIrq0Rq & PHHAL_HW_PN512_BIT_ERRI)
        {
            ...
            /* Fifo write error */
            if (bRegister & PHHAL_HW_PN512_BIT_WRERR)
            {
                status = PH_ERR_READ_WRITE_ERROR;
            }
            /* temperature error */
            else if (bRegister & PHHAL_HW_PN512_BIT_TEMPERR)
            {
                status = PH_ERR_TEMPERATURE_ERROR;
            }
            /* RF error (should not happen) */
            /*
            if (bRegister & PHHAL_HW_PN512_BIT_RFERR)
            {
            status = PH_ERR_RF_ERROR;
            }
            */
            /* buffer overflow */
            else if (bRegister & PHHAL_HW_PN512_BIT_BUFFEROVFL)
            {
                status = PH_ERR_BUFFER_OVERFLOW;
            }
            /* collision */
            else if (bRegister & PHHAL_HW_PN512_BIT_COLLERR)
            {
                ...
                else
                {
                    ...
                    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_PN512_REG_COLL, &bRegister));

                    /* check if collpos is valid */
                    if (!(bRegister & PHHAL_HW_PN512_BIT_COLLPOSNOTVALID))
                    {
                        /* retrieve collpos */
                        bRegister &= PHHAL_HW_PN512_MASK_COLLPOS;
            ...
            /* protocol error */
            else if (bRegister & PHHAL_HW_PN512_BIT_PROTERR)
            {
                status = PH_ERR_PROTOCOL_ERROR;
            }
            /* CRC / parity error */
            else if ((bRegister & PHHAL_HW_PN512_BIT_CRCERR) || (bRegister & PHHAL_HW_PN512_BIT_PARITYERR))
            {
        ...
        (status != PH_ERR_SUCCESS_INCOMPLETE_BYTE))
    {
        ...
        if(pDataParams->bCardType == PHHAL_HW_CARDTYPE_I18092MPT)
        {
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_COMMAND, PHHAL_HW_PN512_CMD_TRANSCEIVE));
        }
    ...
    {
        /* Calculate FDT Value */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_GetFdt(pDataParams, status, &dwTimingSingle));
                                ...
                                )
{
    ...
    /* Retrieve RxDataRate */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_GetConfig(pDataParams, PHHAL_HW_CONFIG_RXDATARATE, &wDataRate));
    wDataRate++;
    ...
    fTimeoutUs = fTimeoutUs + (PHHAL_HW_PN512_TIMER_SHIFT * (PHHAL_HW_PN512_ETU_106 / (float32_t)wDataRate));
    ...
    {
        fTimeoutUs = fTimeoutUs + PHHAL_HW_PN512_FELICA_ADD_DELAY_US;
    }

    /* Max. timeout check */
    if (fTimeoutUs > PHHAL_HW_PN512_TIMER_MAX_VALUE_US)
    {
    ...
    fTemp = (((fTimeoutUs * PHHAL_HW_PN512_TIMER_FREQ) / 65535.0f) - 1.0f) / 2.0f;
    ...
    fTemp = (fTimeoutUs * PHHAL_HW_PN512_TIMER_FREQ) / (1.0f + (2.0f * (float32_t)wPrescaler));
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_TPRESCALER, (uint8_t)(wPrescaler & 0xFF)));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_ReadRegister(pDataParams, PHHAL_HW_PN512_REG_TMODE, &bRegister));
    bRegister &= (uint8_t)~(uint8_t)PHHAL_HW_PN512_MASK_TPRESCALER_HI;
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_TMODE, bRegister | (uint8_t)((wPrescaler >> 8) & 0x0F)));

    /* write Counter value */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_TRELOADLO, (uint8_t)(wReload & 0xFF)));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_TRELOADHI, (uint8_t)((wReload >> 8) & 0xFF)));
                                ...
                                )
{
    ...
    /* Get prescaler value */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_ReadRegister(pDataParams, PHHAL_HW_PN512_REG_TMODE, &bRegister));
    wPsReload = (uint16_t)(bRegister & PHHAL_HW_PN512_MASK_TPRESCALER_HI) << 8;
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_ReadRegister(pDataParams, PHHAL_HW_PN512_REG_TPRESCALER, &bRegister));
    wPsReload |= (uint16_t)bRegister;

    /* Get reload value */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_ReadRegister(pDataParams, PHHAL_HW_PN512_REG_TRELOADHI, &bRegister));
    wTReload = (uint16_t)bRegister << 8;
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_ReadRegister(pDataParams, PHHAL_HW_PN512_REG_TRELOADLO, &bRegister));
    wTReload |= (uint16_t)bRegister;

    /* Get counter value */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_ReadRegister(pDataParams, PHHAL_HW_PN512_REG_TCOUNTERVALHI, &bRegister));
    wTValue = (uint16_t)bRegister << 8;
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_ReadRegister(pDataParams, PHHAL_HW_PN512_REG_TCOUNTERVALLO, &bRegister));
    ...
    fTimeout = ((float32_t)wTValue * (1.0f + (2.0f * (float32_t)wPsReload))) / PHHAL_HW_PN512_TIMER_FREQ;

    /* Subtract timershift from timeout value */
    fTimeout = fTimeout - (PHHAL_HW_PN512_TIMER_SHIFT * (PHHAL_HW_PN512_ETU_106 / (float32_t)wDataRate));
    ...
    *pTime = (uint32_t)fTimeout;
    ...
    if ((wExchangeStatus & PH_ERR_MASK) == PH_ERR_IO_TIMEOUT)
    {
        ...
        {
            if (*pTime > PHHAL_HW_PN512_FELICA_ADD_DELAY_US)
            {
                *pTime = *pTime - PHHAL_HW_PN512_FELICA_ADD_DELAY_US;
            }
        }
    }
                                     ...
                                     )
{
    ...
    {
        /* Redundant code */
        return phhalHw_Pn512_GetRxBuffer(pDataParams, bIsExchange, pTxBuffer, pTxBufferLen, pTxBufferSize);
    }
