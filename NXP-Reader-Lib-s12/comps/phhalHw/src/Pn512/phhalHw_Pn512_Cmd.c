                                     ...
                                     )
{
    ...
    /* Check options */
    if (wOption &
            (uint16_t)~(uint16_t)(PHHAL_HW_PN512_OPTION_RXTX_TIMER_START
                    | PHHAL_HW_OPTION_START_CMD_ONLY | PHHAL_HW_OPTION_FIFO_READ
                    | PHHAL_HW_OPTION_START_TRANSCIEVE))
    {
        status = PH_ADD_COMPCODE(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
    ...
    if (wOption & (PHHAL_HW_PN512_OPTION_RXTX_TIMER_START | PHHAL_HW_OPTION_START_CMD_ONLY))
    {
        ...
        /* Start the command and exit */
        if (wOption & PHHAL_HW_OPTION_START_CMD_ONLY)
        {
            if(wOption & PHHAL_HW_OPTION_START_TRANSCIEVE)
            {
                /* start the transcieve command */
                PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(
                        pDataParams,
                        PHHAL_HW_PN512_REG_COMMAND,
                        PHHAL_HW_PN512_CMD_TRANSCEIVE));
            }
            else
            {
    ...
    if (wOption & (PHHAL_HW_PN512_OPTION_RXTX_TIMER_START | PHHAL_HW_OPTION_FIFO_READ))
    {
                                       ...
                                       )
{
    ...
    /* start the command */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_COMMAND, PHHAL_HW_PN512_CMD_SOFTRESET));
