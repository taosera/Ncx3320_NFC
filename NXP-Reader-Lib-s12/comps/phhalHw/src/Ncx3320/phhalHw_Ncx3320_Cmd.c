                                  ...
                                  )
{
    ...
    /* Check I/Q values */
    if (!(bMode & PHHAL_HW_NCx3320_CMD_LPCD_MODE_OPTION_IGNORE_IQ))
    {
    ...
    /* Parameter check */
    switch (bMode & (uint8_t)~(uint8_t)PHHAL_HW_NCx3320_CMD_LPCD_MODE_OPTION_MASK)
    {
    case PHHAL_HW_NCx3320_CMD_LPCD_MODE_DEFAULT:

        /* start and wait for timer */
        wOption |= PHHAL_HW_NCx3320_OPTION_RXTX_TIMER_START;
        bIrq1WaitFor |= PHHAL_HW_NCx3320_BIT_TIMER1IRQ;

        /* Write default. T3 reload value */
        wReload = PHHAL_HW_NCx3320_LPCD_T3_RELOAD_MIN;
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T3RELOADHI, (uint8_t)(wReload >> 8)));
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T3RELOADLO, (uint8_t)(wReload)));

        /* Write min. T4 reload value */
        wReload = PHHAL_HW_NCx3320_LPCD_T4_RELOAD_MIN;
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T4RELOADHI, (uint8_t)(wReload >> 8)));
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T4RELOADLO, (uint8_t)(wReload)));

        /* Configure T4 for AutoLPCD & AutoRestart */
        bRegister = PHHAL_HW_NCx3320_BIT_T4AUTOLPCD | PHHAL_HW_NCx3320_BIT_T4AUTORESTARTED;
        /* Use 64Khz LFO */
        bRegister |= PHHAL_HW_NCx3320_VALUE_TCLK_LFO_64_KHZ;
        /* Start T4 */
        bRegister |= PHHAL_HW_NCx3320_BIT_T4STARTSTOPNOW | PHHAL_HW_NCx3320_BIT_T4RUNNING;
        /* Set AutoTrimm bit if requested */
        if (bMode & PHHAL_HW_NCx3320_CMD_LPCD_MODE_OPTION_TRIMM_LPO)
        {
            bRegister |= PHHAL_HW_NCx3320_BIT_T4AUTOTRIMM;
        }

        /* Write T4Control register */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T4CONTROL, bRegister));
    ...
    case PHHAL_HW_NCx3320_CMD_LPCD_MODE_POWERDOWN:
        ...
        fReload = (float32_t)wDetectionTimeUs * PHHAL_HW_NCx3320_TIMER0_FREQ;
        if (fReload > (float32_t)PHHAL_HW_NCx3320_TIMER_RELOAD_MAX)
        {
            return PH_ADD_COMPCODE(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
        }
        ...
        bRegister = PHHAL_HW_NCx3320_BIT_T4AUTOLPCD | PHHAL_HW_NCx3320_BIT_T4AUTORESTARTED | PHHAL_HW_NCx3320_BIT_T4AUTOWAKEUP;
        /* Use 2Khz LFO */
        bRegister |= PHHAL_HW_NCx3320_VALUE_TCLK_LFO_2_KHZ;
        /* Start T4 */
        bRegister |= PHHAL_HW_NCx3320_BIT_T4STARTSTOPNOW | PHHAL_HW_NCx3320_BIT_T4RUNNING;
        /* Set AutoTrimm bit if requested */
        if (bMode & PHHAL_HW_NCx3320_CMD_LPCD_MODE_OPTION_TRIMM_LPO)
    ...
    /* Clear LPCD result */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_LPCD_RESULT_Q, PHHAL_HW_NCx3320_BIT_LPCDIRQ_CLR));
    ...
    if (!(bMode & PHHAL_HW_NCx3320_CMD_LPCD_MODE_OPTION_IGNORE_IQ))
    {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_LPCD_QMIN, bRegister));

        /* Set Qmax register */
        bRegister = bQMax | (uint8_t)((bIMax & 0x0C) << 4);
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_LPCD_QMAX, bRegister));

        /* Set Imin register */
        bRegister = bIMin | (uint8_t)((bIMax & 0x03) << 6);
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_LPCD_IMIN, bRegister));
    }

    /* Set Mix2Adc bit */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_RCV, &bRegister));
    bRegister |= PHHAL_HW_NCx3320_BIT_RX_SHORT_MIX2ADC;
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_RCV, bRegister));

    /* Backup current RxAna setting */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXANA, &bRxAnaBackup));
    /* Raise receiver gain to maximum */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXANA, 0x03));

    /* Wait until T4 is started*/
    do
    {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_T4CONTROL, &bRegister));
    }
    while(!(bRegister & PHHAL_HW_NCx3320_BIT_T4RUNNING));
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXANA, bRxAnaBackup));
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_IRQ1, &bRegister));
    if (!(bRegister & PHHAL_HW_NCx3320_BIT_LPCDIRQ))
    {
...
phStatus_t phhalHw_Ncx3320_Cmd_LpcdConfig(
                                        phhalHw_Ncx3320_DataParams_t * pDataParams,
                                        uint8_t * pI,
                                        uint8_t * pQ
                                        )
{
    ...
    /* Set Qmin register */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_LPCD_QMIN, 0xC0));

    /* Set Qmax register */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_LPCD_QMAX, 0xFF));

    /* Set Imin register */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_LPCD_IMIN, 0xC0));

    /* Use default mode, trimm lpo and permit the function to set I and Q values */
    bMode = PHHAL_HW_NCx3320_CMD_LPCD_MODE_DEFAULT | PHHAL_HW_NCx3320_CMD_LPCD_MODE_OPTION_TRIMM_LPO | PHHAL_HW_NCx3320_CMD_LPCD_MODE_OPTION_IGNORE_IQ;

    /* Retrieve current timeout */
    status = phhalHw_Ncx3320_GetConfig(pDataParams, PHHAL_HW_CONFIG_TIMEOUT_VALUE_US, &wTimeoutPrev);
    bTimeoutPrevInMs = 0;
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_SetConfig(pDataParams, PHHAL_HW_CONFIG_TIMEOUT_VALUE_US, 150));

    /* Execute trimming procedure */
    status = phhalHw_Ncx3320_Cmd_Lpcd(
        pDataParams,
        bMode,
        0x00,
        0x00,
    ...
    {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_SetConfig(pDataParams, PHHAL_HW_CONFIG_TIMEOUT_VALUE_US, wTimeoutPrev));
    }
    else
    {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_SetConfig(pDataParams, PHHAL_HW_CONFIG_TIMEOUT_VALUE_MS, wTimeoutPrev));
    }
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_LPCD_RESULT_I, pI));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_LPCD_RESULT_Q, pQ));
...
phStatus_t phhalHw_Ncx3320_Cmd_Receive(
                                     phhalHw_Ncx3320_DataParams_t * pDataParams,
                                     uint16_t wOption,
                                     uint8_t ** ppRxBuffer,
                                     uint16_t * pRxLength
                                     )
{
    ...
    PH_LOG_HELPER_ALLOCATE_TEXT(bFunctionName, "phhalHw_Ncx3320_Cmd_Receive");
    ...
    /* Check options */
    if (wOption & (uint16_t)~(uint16_t)(PHHAL_HW_NCx3320_OPTION_RXTX_TIMER_START))
    {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXWAIT, &bRegRxWait));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXWAIT, 0));
    ...
    if (!(wOption & PHHAL_HW_NCx3320_OPTION_RXTX_TIMER_START))
    {
        ...
        if (!(bRegister & PHHAL_HW_NCx3320_BIT_T1RUNNING))
        {
    ...
    /* start the command */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_COMMAND, PHHAL_HW_NCx3320_CMD_RECEIVE));
    ...
    if (status == PH_ERR_SUCCESS)
    {
        ...
        /* buffer overflow */
        if (bError & PHHAL_HW_NCx3320_BIT_FIFOOVL)
        {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXWAIT, bRegRxWait));
...
phStatus_t phhalHw_Ncx3320_Cmd_Transmit(
                                      phhalHw_Ncx3320_DataParams_t * pDataParams,
                                      uint16_t wOption,
                                      uint8_t * pTxBuffer,
                                      uint16_t wTxLength
                                      )
{
    ...
    PH_LOG_HELPER_ALLOCATE_TEXT(bFunctionName, "phhalHw_Ncx3320_Cmd_Transmit");
    PH_LOG_HELPER_ALLOCATE_PARAMNAME(wOption);
    ...
    /* Check options */
    if (wOption & (uint16_t)~(uint16_t)(PH_EXCHANGE_BUFFERED_BIT | PH_EXCHANGE_LEAVE_BUFFER_BIT | PHHAL_HW_NCx3320_OPTION_RXTX_TIMER_START))
    {
        status = PH_ADD_COMPCODE(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
    ...
    {
        /* Timer 0 */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_T0CONTROL, &bRegister));
        bRegister &= (uint8_t)~(uint8_t)PHHAL_HW_NCx3320_MASK_TSTART;
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T0CONTROL, bRegister));
        /* Timer 1 */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_T1CONTROL, &bRegister));
        bRegister &= (uint8_t)~(uint8_t)PHHAL_HW_NCx3320_MASK_TSTART;
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T1CONTROL, bRegister));
    ...
    else
    {
        ...
        wOption &= (uint16_t)~(uint16_t)PHHAL_HW_NCx3320_OPTION_RXTX_TIMER_START;
    }
    ...
    /* start the command */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_COMMAND, PHHAL_HW_NCx3320_CMD_TRANSMIT));
    ...
    /* Check if an error occured */
    if (bIrq0Reg & PHHAL_HW_NCx3320_BIT_ERRIRQ)
    {
        /* read the error register */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_ERROR, &bError));
    ...
    if (bRestoreTmrSet)
    {
        /* Timer 0 */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_T0CONTROL, &bRegister));
        bRegister |= PHHAL_HW_NCx3320_BIT_TSTART_TX;
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_T0CONTROL, bRegister));
        /* Timer 1 */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_T1CONTROL, &bRegister));
        bRegister |= PHHAL_HW_NCx3320_BIT_TSTART_TX;
    ...
    )
{
    ...
    if ((wAddress >= PHHAL_HW_NCx3320_EEPROM_NUM_PAGES) || (bDataLen == 0) || (bDataLen > PHHAL_HW_NCx3320_EEPROM_PAGE_SIZE))
    {
        return PH_ADD_COMPCODE(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
    }
                                    ...
                                    )
{
    ...
    if (pDataParams->bFifoSize == PHHAL_HW_NCx3320_VALUE_FIFOSIZE_255)
    {
        if (wNumBytes > 0xFF)
        {
            return PH_ADD_COMPCODE(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
                                     ...
                                     )
{
    ...
    if ((wEEAddress < PHHAL_HW_NCx3320_EEPROM_SECTION2_BEGIN) || ((wEEAddress + bNumBytes) > (PHHAL_HW_NCx3320_EEPROM_SECTION2_END + 1)))
    {
        return PH_ADD_COMPCODE(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
    }
                                        ...
                                        )
{
    ...
    /* Parameter check */
    if ((uint16_t)(bKeyNo + bNumberOfKeys) > PHHAL_HW_NCx3320_MAX_NUM_KEYS)
    {
        return PH_ADD_COMPCODE(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
    }

    /* Calculate overal data length */
    wDataLen = (uint16_t)(bNumberOfKeys * PHHAL_HW_NCx3320_MFC_KEY_LENGTH);
                                       ...
                                       )
{
    ...
    return phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_COMMAND, PHHAL_HW_NCx3320_CMD_SOFTRESET);
}
                                    ...
                                    )
{
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXCORR, 0x88));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXCTRL, 0x07));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_RXWAIT, 0x3D));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_TXWAITLO, 0x80));
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Ncx3320_WriteFifo(pDataParams, bBuffer, 0x03, &wBytesWritten));

    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_NCx3320_REG_COMMAND, PHHAL_HW_NCx3320_CMD_ACKREQ));

    do
    {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_NCx3320_REG_STATUS, &bReg));
    }while(bReg != 0x00);
